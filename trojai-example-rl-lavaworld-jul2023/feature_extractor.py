...
def get_model_features(model_filepath):
    ...
    all_backbone_params = []
    for param in model_backbone.actor.parameters():
        all_backbone_params.append(param.data.cpu().numpy())
    for param in model_backbone.critic.parameters():
        all_backbone_params.append(param.data.cpu().numpy())

    features = []

    if model_class == 'BasicFCModel':
        features = _get_eigen_vals_fc(all_backbone_params, 0, 6)
    elif model_class == 'SimplifiedRLStarter':
        features = _get_eigen_vals_fc(all_backbone_params, 0, 3)
    else:
        print(model_class, " is an unknown model!!!!!!")
