...
class FasterRCNN(torchvision.models.detection.generalized_rcnn.GeneralizedRCNN):
    ...
    def basic_forward(self, images, targets):
        ...
        cls_logits = self.roi_heads(features, proposals, images.image_sizes, targets)
    ...
    def forward(self, images, targets=None):
        ...
        cls_logits = self.basic_forward(images, targets)
...
def ssd300_vgg16(
    ...
    **kwargs: Any,
    ...
    # _vgg_extractor - # Gather the indices of maxpools. These are the locations of output blocks.
...
class SSD(torchvision.models.detection.SSD):
    ...
    def basic_forward(self, images, targets):
        ...
        # head_outputs['cls_logits'].shape - torch.Size([1, 8732, 91]). 8732 boxes per class
        # head_outputs['bbox_regression'].shape - torch.Size([1, 8732, 4]). 4 offsets relative to the original default box shape

        # create the set of anchors
        anchors = self.anchor_generator(images, features)
        ## anchors[0].shape - (8732, 4)
        ...
        return loss_dict, detections, head_outputs['cls_logits']
    ...
    def forward(self, images: List[torch.Tensor], targets: Optional[List[Dict[str, torch.Tensor]]] = None) -> Tuple[Dict[str, torch.Tensor], List[Dict[str, torch.Tensor]]]:
        ...
        losses, detections, head_outputs_cls_logits = self.basic_forward(images, targets)
        # detections = self.transform.postprocess(detections, images.image_sizes, original_image_sizes)

        if targets is None:
            return head_outputs_cls_logits
        else:
            return losses, detections, head_outputs_cls_logits
